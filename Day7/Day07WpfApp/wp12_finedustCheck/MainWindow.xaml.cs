...
namespace wp12_finedustCheck
{
    ...
    public partial class MainWindow : MetroWindow
    {
        ...
        private async void BtnReqRealtime_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                if (status == 200) // 정상이면 데이터받아서 처리
                {
                    ...
                    var dustSeneors = new List<DustSensor>();
                    foreach (var sensor in json_array)
                    {
                        dustSeneors.Add(new DustSensor()
                        {
                            Id = 0,
                    ...
                    this.DataContext = dustSeneors;
                    StsResult.Content = $"OpenAPI {dustSeneors.Count}건 조회완료";
                }
            }
            catch (Exception ex)
            {
        ...
        private async void BtnSaveData_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                await Commons.ShowMessageAsync("오류", "그... 없어요!");
                return;
            }
            ...
            {
                await Commons.ShowMessageAsync("실패", $"DB 저장 실패!{ex.Message}");
            }
        }
            ...
            if (CboReqDate.SelectedValue != null)
            {
                ...
                using (MySqlConnection conn = new MySqlConnection(Commons.myConnString))
                {
                    ...
                    foreach (DataRow row in ds.Tables["dustsensor"].Rows)
                    {
                        ...
                        {
                            Id = Convert.ToInt32(row["Id"]), // mysql 컬럼이름에 대소문자 구분없이 씀
